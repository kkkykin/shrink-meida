...
- [x] 封装 `OpenListManager`（仅 server 使用）
  - [x] `get_download_url(path) -> {url, expires_at?}`（基于 `info.sign` 拼 `/d`）
  - [x] `get_direct_upload_info(dst_path, size) -> {url, method, chunk_size, headers?, expires_at}`（调用 `/api/fs/get_direct_upload_info`）
  - [x] `ensure_dir(path)`
  - [x] `info(path)` / `listdir(refresh=True)`（用于 verify）
  - [x] `rename(src, dst)` / `remove(path)`（用于 finalize）
- [x] 实现 **finalize**：
  - [x] 校验 staging size == expected
  - [x] 计算最终 `dst_path`（`dst_path = route.out_root + dst_rel`）
  - [x] overwrite/冲突策略：
    - 默认：若目标存在且 size 相同 → 视为已完成（幂等）
    - 若目标存在且 size 不同 → 进入 fail（后续可加 `__2/__3` 策略）
  - [x] rename staging → final
  - [x] 再次 verify final size
